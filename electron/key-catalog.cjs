...
async function fetchKeyCatalog(){
  ...
  for(const map of maps){const localized=mapById.get(map.id),doorKeys=uniqueBy(values(map.locks).map(usableLockKey).filter(Boolean),id=>id),accessKeys=uniqueBy(values(map.accessKeys).map(key=>typeof key==="string"?key:key?.id).filter(Boolean),id=>id);for(const [kind,keyIds] of [["door",doorKeys],["access",accessKeys]])for(const keyId of keyIds){const entries=mapUseByKey.get(keyId)||[];entries.push({id:map.id,name:localized?.name||map.id,nameEn:localized?.nameEn||map.id,kind});mapUseByKey.set(keyId,entries)}}
