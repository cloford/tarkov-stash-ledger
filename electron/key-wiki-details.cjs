...
const WIKI_FIELDS=["lockLocation","lockLocationEn","behindLock","behindLockEn","wikiUpdatedAt"];
...
async function enrichKeyCatalogWithWiki(catalog,{translate=true}={}){
  const keys=Array.isArray(catalog?.keys)?catalog.keys:[],mapDirectory=[...new Map(keys.flatMap(key=>key.mapUses||[]).map(map=>[map.nameEn,map])).values()].sort((a,b)=>String(b.nameEn).length-String(a.nameEn).length),titles=keys.map(key=>wikiTitleFromUrl(key.wiki)),pages=await fetchWikiPages(titles),details=keys.map((key,index)=>{
    const title=titles[index],page=pages.get(compactTitle(title)),wikitext=page?.revisions?.[0]?.slots?.main?.content||"",lockLocationEn=extractWikiSection(wikitext,"Lock Location")||extractInfoboxUsage(wikitext),behindLockEn=extractWikiSection(wikitext,"Behind the Lock");
    return{key,lockLocationEn,behindLockEn};
  });
  ...
  return{...catalog,wikiUpdatedAt,keys:details.map(({key,lockLocationEn,behindLockEn})=>{const ja=jaById.get(key.id)||{},mapUses=[...(key.mapUses||[])];for(const map of mapDirectory){const name=String(map.nameEn||""),aliases=name==="The Lab"?["The Lab","TerraGroup Labs"]:[name],mentioned=aliases.some(alias=>new RegExp(`(?:^|[^a-z])${alias.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}(?:$|[^a-z])`,"i").test(lockLocationEn));if(mentioned&&!mapUses.some(entry=>entry.id===map.id))mapUses.push({id:map.id,name:map.name,nameEn:map.nameEn,kind:"wiki",positions:[]})}return{...key,lockLocation:ja.lockLocation||lockLocationEn,lockLocationEn,behindLock:ja.behindLock||behindLockEn,behindLockEn,wikiUpdatedAt,mapUses}})};
}
...
function mergeCatalogWikiDetails(catalog,...sources){
  ...
  return{...catalog,wikiUpdatedAt:sources.find(source=>source?.wikiUpdatedAt)?.wikiUpdatedAt||null,keys:(catalog?.keys||[]).map(key=>{const saved=byId.get(key.id)||{},detail={};for(const field of WIKI_FIELDS)if(saved[field])detail[field]=saved[field];const mapUses=[...(key.mapUses||[])];for(const map of saved.mapUses||[])if(map.kind==="wiki"&&!mapUses.some(entry=>entry.id===map.id))mapUses.push(map);return{...key,...detail,mapUses}})};
}
